...
public class CognitoPlugin: AuthCategoryPlugin {
    ...
    public func fetchAuthSession(options: AuthFetchSessionRequest.Options?) async throws
        -> any AuthSession
    {
            ...
            DispatchQueue.main.async {
                self.nativeAuthPlugin.fetchAuthSession { result in
                    ...
                    case .failure(let error):
                        let returning = NativeAWSAuthCognitoSession(
                            from: NativeAuthSession(isSignedIn: false))
                        continuation.resume(returning: returning)
                    }
                }
            }
    ...
    public func getCurrentUser() async throws -> any AuthUser {
        guard let user = currentUser else {
            throw FlutterError(
                code: "NO_CURRENT_USER", message: "No current user is signed in.", details: nil)
        }
        return user
    }

    public func signUp(username: String, password: String?, options: AuthSignUpRequest.Options?)
    ...
    public func confirmSignUp(
        for username: String, confirmationCode: String, options: AuthConfirmSignUpRequest.Options?
    ) async throws -> AuthSignUpResult {
        preconditionFailure("method not supported")
    }

    public func resendSignUpCode(
        for username: String, options: AuthResendSignUpCodeRequest.Options?
    ) async throws -> AuthCodeDeliveryDetails {
        preconditionFailure("method not supported")
    }

    public func signIn(username: String?, password: String?, options: AuthSignInRequest.Options?)
    ...
    public func signInWithWebUI(
        presentationAnchor: AuthUIPresentationAnchor?, options: AuthWebUISignInRequest.Options?
    ) async throws -> AuthSignInResult {
        preconditionFailure("method not supported")
    }

    public func signInWithWebUI(
        for authProvider: AuthProvider, presentationAnchor: AuthUIPresentationAnchor?,
        options: AuthWebUISignInRequest.Options?
    ) async throws -> AuthSignInResult {
        preconditionFailure("method not supported")
    }

    public func confirmSignIn(challengeResponse: String, options: AuthConfirmSignInRequest.Options?)
        async throws -> AuthSignInResult
    {
        preconditionFailure("method not supported")
    }
    ...
    public func resetPassword(for username: String, options: AuthResetPasswordRequest.Options?)
    ...
    public func confirmResetPassword(
        for username: String, with newPassword: String, confirmationCode: String,
    ...
    public func fetchUserAttributes(options: AuthFetchUserAttributesRequest.Options?) async throws
        -> [AuthUserAttribute]
    {
        preconditionFailure("method not supported")
    }
    ...
    public func update(
        userAttribute: AuthUserAttribute, options: AuthUpdateUserAttributeRequest.Options?
    ) async throws -> AuthUpdateAttributeResult {
        preconditionFailure("method not supported")
    }

    public func update(
        userAttributes: [AuthUserAttribute], options: AuthUpdateUserAttributesRequest.Options?
    ...
    public func confirm(
        userAttribute: AuthUserAttributeKey, confirmationCode: String,
    ...
    public func update(
        oldPassword: String, to newPassword: String, options: AuthChangePasswordRequest.Options?
    ) async throws {
        preconditionFailure("method not supported")
    }

    public func fetchDevices(options: AuthFetchDevicesRequest.Options?) async throws
        -> [any AuthDevice]
    {
        preconditionFailure("method not supported")
    }

    public func forgetDevice(_ device: (any AuthDevice)?, options: AuthForgetDeviceRequest.Options?)
        async throws
    {
        preconditionFailure("method not supported")
    }
